...
class MonModel extends MongoModel {
     ...
     * )
     */
    public function __construct($config = array('host' => '', 'port' => 27017, 'username' => '', 'password' => '', 'db' => 'ichunt',  'cmd' => '$')){

        $config = array(
            'host'      =>  C('DB_HOST'), //ip
            'port'      =>  C('DB_PORT'), //端口号
            'username'  =>  C('DB_NAME'), //用户名
            'password'  =>  C('DB_PWD'), //密码
            'db'        =>  C('DB_NAME'), //数据库名称
            );

        //$server = "mongodb://".$config['host'].":".$config['port']."/".$config['db'];
		$server = "mongodb://".$config['username'].":".$config['password']."@".$config['host'].":".$config['port']."/".$config['db'];
        try {
            $this->_mongo = new \MongoClient($server);// 立即连接
