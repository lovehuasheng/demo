...
class IndexController extends Controller {
    ...
    public function index(){
        ...
        $q = urldecode(I('get.keyword')); //型号关键词
        ...
        if(is_array($array) && count($array)) {
            ...
            if(is_array($trueArray) && count($trueArray)){ //有结果
                ...
                if(isset($trueArray['Availability']) && is_string($trueArray['Availability'])){ //只有一个结果
                    ...
                    $pdTemp['goods_name_temp'] = str_ireplace($keyword, "<b class='f-red'>".$keyword."</b>",$pdTemp['goods_name']);
                    ...
                    if(isset($_GET['kNums']) && $_GET['kNums']<=$pdTemp['stock'][1]){
...
//                    unset($pd);
    ...
    public function updateMonAndMysql($goods_list,$goods_sn_arr){
            ...
            foreach($res as $k=>$v){
                if(isset($goods_list[$v['goods_sn']]) && !empty($goods_list[$v['goods_sn']])){
                    ...
                    if(empty($update_data)){
                        ...
                        if(isset($goods_list[$v['goods_sn']]['tiered'][0][0]) && $goods_list[$v['goods_sn']]['tiered'][0][0]>0 && isset($goods_list[$v['goods_sn']]['stock'][1]) && $goods_list[$v['goods_sn']]['stock'][1]>0){
                            array_unshift($update_data, $goods_list[$v['goods_sn']]);
                        }else{
                            array_push($update_data, $goods_list[$v['goods_sn']]);
                        }
                    }
        ...
        if($insert_res){
            $re_data = array_merge($update_data,$insert_res);
        }else{
            $re_data = $update_data;
        }
        return $re_data;
...
//            $sql = 'show tables';
    ...
    public function test(){exit;
        ...
        dump($res);
        $re = $M->update('mouser',array('time'=>0),$arr);
        $res = $M->select('mouser',$arr,array('goods_id','goods_name','brand_name','time'),array()); //根据数据表及条件查询结果
        dump($res);
    }
    public function ddd(){
