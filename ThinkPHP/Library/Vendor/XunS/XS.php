...
class XSIndex extends XSServer
{
	...
	public static function getTermWeightMultiple($value,$term)
	{
		if($value == $term){
			return 50;
		}
		if(is_array($term)){
			...
			similar_text($value,$term[0],$parent);
			return (10 + 10 * $parent/100) * strlen($term[0]) / strlen($value);
		}
	    return 1;
	} 
			...
			if (($value = $doc->f($field)) !== null) {
				...
				if (!$field->hasCustomTokenizer()) {
					...
					if ($field->hasIndex()) {
						...
						$isChipstep = false;
						$hasSplit = strpos($value,'€€') !==  false ? true : false;
						if(class_exists('XSTokenizerChipstep') and $obj instanceof XSTokenizerChipstep){
							$isChipstep = true;
						}
						$terms = $obj->getTokens($value, $doc);
						if ($field->hasIndexSelf()) {
							$wdf = $field->isBoolIndex() ? 1 : ($field->weight | CMD_INDEX_FLAG_CHECKSTEM);
							if($hasSplit && $isChipstep){
								$count = count(explode('€€',$value));
								$tmp_terms = array();
								for($i=0;$i<$count;$i++){
									$tmp_terms = array_merge($tmp_terms,$terms[$i]);
								}
								$terms = $tmp_terms;
							}
						...
						if ($field->hasIndexMixed()) {
							if($isChipstep){
								...
								$value_arr_count = count($value_arr);
								foreach($value_arr as $k => $val){		
									$split_terms = $value_arr_count > 1 ? $terms[$k] : $terms;
									$count = count($split_terms);
									foreach ($split_terms as $term) {
										...
										$term = strtolower($term);
										$cmds[] = new XSCommand(CMD_DOC_INDEX, $weight, XSFieldScheme::MIXED_VNO, $term);
									}
								}
							}else{
								$mtext = implode(' ', $terms);
